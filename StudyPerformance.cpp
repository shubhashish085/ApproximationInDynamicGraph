...
void countSquareForThinkDInIncrementalGraphStream(const std::string& file_path, const std::string& output_file, ThinkDFD*& module, ui interval){
    
    std::ifstream infile(file_path);
    std::ofstream outputfile;
    std::string final_output_file =  "Inc_" + output_file;
    outputfile.open(final_output_file, std::ios::app);
...
int main(int argc, char** argv){
    ...
    ThinkDFD* module = new ThinkDFD(memory_budget, lowerbound);    
    countSquareForThinkDInIncrementalGraphStream(input_data_graph_file, output_file, module, interval);    
